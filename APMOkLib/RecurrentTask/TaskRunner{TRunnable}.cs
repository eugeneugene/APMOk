    ...
    where TRunnable : IRunnable
{
    ...
    public TaskRunner(ILoggerFactory loggerFactory, TaskOptions<TRunnable> options, IServiceScopeFactory serviceScopeFactory)
    {
        ...
        logger.LogTrace("Instance of {name}<{fullname}> is created", nameof(TaskRunner<TRunnable>), typeof(TRunnable).FullName);
    }
    ...
    public void Start(CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation("<{name}>.Start() called...", RunnableType.Name);
        if (mainTask != null)
    ...
    public void Stop()
    {
        logger.LogInformation("<{name}>.Stop() called...", RunnableType.Name);
        if (mainTask is null)
    ...
    {
        logger.LogInformation("<{name}> started. Running...", RunnableType.Name);
        var sleepInterval = firstRunDelay;
        var handles = new[] { cancellationToken.WaitHandle, runImmediately };
        while (true)
        {
            logger.LogDebug("<{name}> Sleeping for {interval}...", RunnableType.Name, sleepInterval);
            ...
            logger.LogDebug("<{name}> Creating scope...", RunnableType.Name);
            using (var scope = ServiceScopeFactory.CreateScope())
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        logger.LogInformation("Calling <{name}>.RunAsync()...", RunnableType.Name);
                ...
                {
                    logger.LogError("Error in <{name}>.RunAsync() (ignoring, see RunStatus.LastException or handle AfterRunFail event):", RunnableType.Name);
                    logger.LogError("{ex}", ex);
        ...
        logger.LogInformation("<{name}>.RunAsync() finished.", RunnableType.Name);
    }
