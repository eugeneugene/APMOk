...
namespace APMOkLib.RecurrentTasks
{
        ...
        where TRunnable : IRunnable
    {
        ...
        public TaskRunner(ILoggerFactory loggerFactory, TaskOptions<TRunnable> options, IServiceScopeFactory serviceScopeFactory)
        {
            ...
            logger.LogTrace("Instance of {0}<{1}> is created", nameof(TaskRunner<TRunnable>), typeof(TRunnable).FullName);
        }
        ...
        protected void MainLoop(TimeSpan firstRunDelay, CancellationToken cancellationToken)
        {
                ...
                using (var scope = ServiceScopeFactory.CreateScope())
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            logger.LogInformation("Calling <{0}>.RunAsync()...", RunnableType.Name);
                    ...
                    {
                        logger.LogError("Error in <{0}>.RunAsync() (ignoring, see RunStatus.LastException or handle AfterRunFail event):", RunnableType.Name);
                        logger.LogError(ex.ToString());
            ...
            logger.LogInformation("<{0}>.RunAsync() finished.", RunnableType.Name);
        }
