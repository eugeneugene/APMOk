...
internal class JsonStringEnumMemberConverter<T> : JsonConverter<T>
{
    ...
    public override T Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        if (token == JsonTokenType.String)
        {
            ...
            if (_TransformedToRaw.TryGetValue(enumString, out EnumInfo? enumInfo1))
                return (T)Enum.ToObject(_EnumType, enumInfo1.RawValue);
            ...
            if (_IsFlags)
            {
                ...
                {
                    // Case sensitive search attempted first.
                    if (_TransformedToRaw.TryGetValue(flagValue, out EnumInfo? enumInfo2))
                        calculatedValue |= enumInfo2.RawValue;
                    else
                    {
